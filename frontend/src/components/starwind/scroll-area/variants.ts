...
export const scrollArea = tv({
  slots: {
    ...
    viewport:
      'starwind-scroll-area-viewport focus-visible:ring-outline/50 size-full overflow-auto rounded-[inherit] transition-[color,box-shadow] outline-none focus-visible:ring-[3px] focus-visible:outline-1 motion-reduce:transition-none',
    content: 'starwind-scroll-area-content min-w-fit',
    corner: 'starwind-scroll-area-corner bg-background absolute end-0 bottom-0 z-10 hidden size-2.5',
  },
});

export const scrollBar = tv({
  base: [
    'starwind-scroll-area-scrollbar absolute z-10 flex touch-none p-px transition-colors select-none motion-reduce:transition-none',
